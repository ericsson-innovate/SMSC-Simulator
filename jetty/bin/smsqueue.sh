        ...
        if [ "$DIST_OS" = "solaris" ] ; then
            ...
            if [ -f /etc/rc.d/$APP_NAME ] ; then
                ...
                if [ -f ${REALDIR}/${APP_NAME}.install ] ; then
                    ...
                    echo 'start_cmd="adm-dpc-manager_start"' >> /etc/rc.d/$APP_NAME
                    echo 'load_rc_config $name' >> /etc/rc.d/$APP_NAME
                    echo 'status_cmd="adm-dpc-manager_status"' >> /etc/rc.d/$APP_NAME
                    echo 'stop_cmd="adm-dpc-manager_stop"' >> /etc/rc.d/$APP_NAME
